...
Steps performed:
  1. Bump version in package.json and package-lock.json and commit the change
...
bump_versions() {
    ...
    if git diff --cached --quiet; then
        ...
        echo "Bumped package.json and package-lock.json to ${ver}"
    fi
}
...
if [[ "$DRY_RUN" -eq 1 ]]; then
    echo "(dry-run) bump package.json and package-lock.json to ${VERSION_NUM}"
